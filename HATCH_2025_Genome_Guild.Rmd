...
server <- function(input, output, session) {
  ...
  observeEvent(input$upload_data, {
    ...
    # Read the uploaded data (assuming fasta or fa file)
    file <- input$file_upload
    if (tools::file_ext(file$name) == "fasta") {
      uploaded_data <- read.csv(file$datapath)
    } else if (tools::file_ext(file$name) == "fa") {
      uploaded_data <- read.table(file$datapath)
    } else {
      output$message <- renderText("Invalid file format. Please upload a fasta (.fasta or .fa) file.")
      return()
    }
    ...
    output$message <- renderText({
      ...
      #source_python("AsymmetricEncryption.py")
      system("bash encryption__CL.sh file")
