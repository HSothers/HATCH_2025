...
otp_storage <- reactiveValues(otp = NULL, user = NULL)
# Define UI
ui <- fluidPage(
  theme = shinytheme("cerulean"),  # Add a theme for a modern look
  titlePanel("WormHole Login Page - 2FA Enabled"),
  sidebarLayout(
    sidebarPanel(
  ...
  observeEvent(input$login, {
    ...
    if (user %in% users$username) {
      ...
      if (pass == stored_pass) {
        ...
        message <- paste("Your OTP is:", otp_storage$otp)
        # In a real app, send email via SMTP (e.g., mailR, Rook)
